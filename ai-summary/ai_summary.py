...
def summarize_assignment_outcome(client, db_path, limit=None, debug=False):
    ...
    for assignment_id, outcome_id, comment, course_title, outcome_name in rows:
        if not comment:
            if debug: print(f"⚠️ Skipped empty comment for assignment_id={assignment_id}, outcome_id={outcome_id}")
            continue
        try:
...
def summarize_grouped(client, db_path, group_by, group_limit=None, debug=False):
    ...
    for label, all_comments in rows:
        if not all_comments or len(all_comments.strip()) < 5:
            ...
            continue
        if len(all_comments) > 12000:  # Rough upper token estimate
            print(f"⚠️ Context size too large for {group_by}={label} (length={len(all_comments)} chars). Skipping.")
            continue
        try:
...
def generate_group_summary(client, label, comments, group_by, debug=False):
    ...
    try:
        ...
        summary = response.choices[0].message.content.strip()
        if debug:
            print(f"🔹 [{group_by}] Summary for {label[:30]}...: {summary[:60]}...")
        return summary
    except Exception as e:
        raise RuntimeError(f"OpenAI API error during grouped summary for {label}: {e}")
