...
import { fetchAISummaries } from './lib/db';
...
// Add this interface for the AI summary
interface AISummary {
  summary_text: string;
  outcome_name: string;
  last_updated: string;
...
export default function FeedbackPlatform() {
  ...
  // Add useEffect to fetch AI summaries when selectedHC changes
  useEffect(() => {
    const loadSummaries = async () => {
      try {
        setError(null); // Clear any previous errors
        const summaries = await fetchAISummaries(selectedHC || 'Organization');
                  ...
                  <CardContent className="p-6">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-2 mb-4">
                        <Lightbulb className="h-5 w-5 text-[#8B6BF2]" />
                        <h3 className="font-semibold text-[#0F172A]">AI Generated Summary:</h3>
                      </div>
                      {error ? (
                        <motion.div
                          className="bg-white rounded-lg p-4 shadow-lg border border-red-200"
                          initial={{ opacity: 0 }}
                          animate={{ opacity: 1 }}
                        >
                        ...
                        aiSummaries.map((summary, index) => (
                          <motion.div
                            key={index}
                            className="bg-white rounded-lg p-6 shadow-lg border border-[#E2E8F0]"
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                        ...
                        <motion.div
                          className="bg-white rounded-lg p-4 shadow-lg border border-[#E2E8F0]"
                          initial={{ opacity: 0 }}
                          animate={{ opacity: 1 }}
                        >
                          <p className="text-sm text-[#334155]">
                            No summary available for this HC/LO yet.
                          </p>
                        </motion.div>
                      )}
                    </div>
